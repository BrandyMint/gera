class Admin::PaymentSystemsController < Admin::ApplicationController
  ...
  def index
    render locals: {
      payment_systems: Gera::PaymentSystem.order(:id),
  ...
  def show
    render locals: {
      payment_system: Gera::PaymentSystem.find(params[:id]),
      columns: SHOW_COLUMNS
    }
  end
  ...
  def new
    render locals: {
      payment_system: Gera::PaymentSystem.new,
      columns: EDIT_COLUMNS
    }
  end

  def create
    Gera::PaymentSystem.create! permitter_params
  ...
  def update
    respond_to do |format|
      if payment_system.update_attributes permitter_params
        format.html { redirect_to(admin_payment_systems_path, :notice => 'Status was successfully updated.') }
